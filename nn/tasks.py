...
# RDPD + SDAF — custom modules for this project
from .rdpd_sdaf import RDPD, SDAF
...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, Detect):  # detection head exposes stride/anchors/strides
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # Input: tensor [B,c1,H,W], Output: packed [B,c2+1,H/2,W/2]
            # where packed[:,:c2] = downsampled feature, packed[:,c2:] = detail prior
            c1 = ch[f] if isinstance(f, int) else ch[f[0]]
            requested_c2 = args[0]
            if requested_c2 != nc:  # if not output classes
                requested_c2 = make_divisible(min(requested_c2, max_channels) * width, 8)
            args = [c1, requested_c2, *args[1:]]
            # RDPD real output = c2 feature channels + 1 prior channel
            c2 = requested_c2 + 1
        elif m is SDAF:
            # SDAF: Semantic-Detail Adaptive Frequency Fusion
            # Receives 2 or 3 inputs: [high_semantic, lateral_detail, (optional rdpd_packed)]
            assert isinstance(f, list), f"SDAF requires list 'from', got {type(f)}"
            assert len(f) in (2, 3), f"SDAF requires 2 or 3 inputs, got {len(f)}"
            inc = [ch[x] for x in f]
            requested_c2 = args[0]
            if requested_c2 != nc:
                requested_c2 = make_divisible(min(requested_c2, max_channels) * width, 8)
            args = [inc, requested_c2, *args[1:]]
            c2 = requested_c2
...
def guess_model_scale(model_path):
    ...
    try:
        ...
        if m:
            return m.group(2)
        m = re.search(r"collabod[-_]([nslmx])", stem, re.IGNORECASE)
        if m:
            return m.group(1)
        return ""
